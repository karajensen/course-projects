...
class FileReaderWidget(QWidget):
    def __init__(self, parent=None, startDir="", runIn="python"):
        ...
        self.folder_le.setContextMenuPolicy(Qt.CustomContextMenu)
        self.folder_le.customContextMenuRequested.connect(self.textBrowserRightClick)
        
        self.folder_le.setText(startDir)
        self.fillList()
       
    def textBrowserRightClick(self, pos):
        newPos = QPoint(pos.x()+20, pos.y())
        menu = self.folder_le.createStandardContextMenu()
        menu.addSeparator()
        menu.addAction("print hello", lambda : self.printer("hello") )
        menu.addSeparator()
        menu.addAction("print goodbye", lambda : self.printer("goodbye") )
