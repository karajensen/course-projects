...
uiFile = os.path.join(os.path.dirname(__file__),"finalFileReader.ui")
...
class FileReaderWin(QMainWindow):
    ...
    def readSettings(self):
        print "reading"
        settings = QSettings("ChocoINC", "Final File Reader")
        
        if type(settings.value("mainWindow/pos")) != type(QVariant()):
            self.move(settings.value("mainWindow/pos"))
        if type(settings.value("mainWindow/size")) != type(QVariant()):
