    ...
    m_data(data)
{
    ...
    {
        LogError("Area patch size must be a cube");
    }
...
void SceneUpdater::Initialise(const glm::vec3& camera)
{
    ...
    const glm::vec2 sandStart(camera.x - sandOffset, camera.z - sandOffset);

    m_waterOffset = water.Size();
    const float waterOffset = (halfPatch * m_waterOffset) - (m_waterOffset / 2.0f);
    const glm::vec2 waterStart(camera.x - waterOffset, camera.z - waterOffset);

    int instance = 0;
    glm::vec2 waterPosition, sandPosition;
